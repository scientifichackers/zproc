...
class TestServerPing(unittest.TestCase):
    ...
    def test_ping(self):
        self.assertEqual(self.ctx.ping(), self.ctx.server_proc.pid)
        self.assertEqual(self.ctx.state.ping(), self.ctx.server_proc.pid)

    def test_timeout(self):
        self.assertEqual(self.ctx.ping(timeout=1), self.ctx.server_proc.pid)
        self.assertEqual(self.ctx.state.ping(timeout=1), self.ctx.server_proc.pid)
