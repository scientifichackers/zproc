...
class Client:
    ...
    def __init__(
        ...
        **process_kwargs
        ...
        self._state = StateMethods(self.server_address)
        if value is not None:
            self.merge(value)

        self.ping = self._state.ping
        self.time = self._state.time

        self.when = self._state.when
        self.when_available = self._state.when_available

        self.when_change = self._state.when_change
        self.when_change_raw = self._state.when_change_raw

        self.when_equal = self._state.when_equal
        self.when_not_equal = self._state.when_not_equal

        self.when_falsy = self._state.when_falsy
        self.when_truthy = self._state.when_truthy

        self.when_none = self._state.when_none
        self.when_not_none = self._state.when_not_none

        self.spawn = self._process.spawn
        self.spawn_map = self._process.spawn_map
        self.wait = self._process.wait
        self.start_all = self._process.start_all
        self.stop_all = self._process.stop_all
        self.ping = self._process.ping
